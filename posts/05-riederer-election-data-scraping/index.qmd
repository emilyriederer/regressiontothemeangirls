...
I think a lot of people know about basic web scraping already, so here, I will demonstrate my favorite and lesser used pattern: headless browser automation. A nice trick can be to use the python `Playwright` library to navigate a browser -- programmatically telling it what buttons to click, files to download, etc. just as if you were moving through the page yourself. 

To navigate and download files from the [Texas Secretary of State], I wanted to follow the steps illustrated below:

![](tx.gif)

This is accomplished by this relatively minial [script](https://github.com/emilyriederer/data-scraping-states/blob/main/04-browserauto-tx.py).
